...
from Resources import fileDialog
...
class GAIT():
    def __init__(self):
        ...
        # Grab Initial Frame From a UI 
        self._FileApp = QApplication(sys.argv)
        self._FileWindow = fileDialog.fileWindow()
        self._InitImageFileName = self.getFiles()
        # Confirmation Box UI
    ...
    def getFiles(self) -> str: 
        self._FileWindow.show()
        self._FileApp.exec_()
        return self._FileWindow.getFileName()
    ...
    def handleInitImg(self): 
        ...
        if not os.path.exists((os.path.join(self._ProgramPath, "init_images"))): 
